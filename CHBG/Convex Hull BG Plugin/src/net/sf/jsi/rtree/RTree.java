...
public class RTree implements SpatialIndex, Serializable {
	 ...
	 */
	private Node splitNode(Node n, float newRectMinX, float newRectMinY, float newRectMaxX, float newRectMaxY,
			int newId) {
		...
		pickSeeds(n, newRectMinX, newRectMinY, newRectMaxX, newRectMaxY, newId, newNode); // this
																							// also
																							// sets
																							// the
																							// entryCount
		...
		while (n.entryCount + newNode.entryCount < maxNodeEntries + 1) {
			...
			if (maxNodeEntries + 1 - n.entryCount == minNodeEntries) {
				...
				for (int i = 0; i < maxNodeEntries; i++) {
					if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED) {
						entryStatus[i] = ENTRY_STATUS_ASSIGNED;
						newNode.addEntry(n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i],
								n.ids[i]);
	 ...
	 */
	private void pickSeeds(Node n, float newRectMinX, float newRectMinY, float newRectMaxX, float newRectMaxY,
			int newId, Node newNode) {
		...
		for (int i = 0; i < n.entryCount; i++) {
			...
			if (log.isDebugEnabled()) {
				log.debug("Entry " + i + ", dimension X: HighestLow = " + tempHighestLow +
						...
						tempLowestHigh + " (index " + tempLowestHighIndex + ", NormalizedSeparation = "
	...
	private int pickNext(Node n, Node newNode) {
		...
		for (int i = 0; i < maxNodeEntries; i++) {
			if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED) {
				...
				float nIncrease = Rectangle.enlargement(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY,
						n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i]);
				float newNodeIncrease = Rectangle.enlargement(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX,
						newNode.mbrMaxY,
						n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i]);
				...
				if (difference > maxDifference) {
					...
					if (nIncrease < newNodeIncrease) {
						...
						nextGroup = 1;
					} else if (Rectangle.area(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY) < Rectangle
							.area(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX, newNode.mbrMaxY)) {
						nextGroup = 0;
					} else if (Rectangle.area(newNode.mbrMinX, newNode.mbrMinY, newNode.mbrMaxX,
							newNode.mbrMaxY) < Rectangle.area(n.mbrMinX, n.mbrMinY, n.mbrMaxX, n.mbrMaxY)) {
						nextGroup = 1;
		...
		if (nextGroup == 0) {
			...
			// move to new node.
			newNode.addEntry(n.entriesMinX[next], n.entriesMinY[next], n.entriesMaxX[next], n.entriesMaxY[next],
					n.ids[next]);
			n.ids[next] = -1;
		}
	...
	private float nearest(Point p, Node n, float furthestDistanceSq, TIntArrayList nearestIds) {
		for (int i = 0; i < n.entryCount; i++) {
			float tempDistanceSq = Rectangle.distanceSq(n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i],
					n.entriesMaxY[i], p.x, p.y);
	...
	private boolean intersects(Rectangle r, TIntProcedure v, Node n) {
		for (int i = 0; i < n.entryCount; i++) {
			if (Rectangle.intersects(r.minX, r.minY, r.maxX, r.maxY, n.entriesMinX[i], n.entriesMinY[i],
					n.entriesMaxX[i], n.entriesMaxY[i])) {
				if (n.isLeaf()) {
					if (!v.execute(n.ids[i])) {
						return false;
	...
	private Node chooseNode(float minX, float minY, float maxX, float maxY, int level) {
		...
		while (true) {
			...
			float leastEnlargement = Rectangle.enlargement(n.entriesMinX[0], n.entriesMinY[0], n.entriesMaxX[0],
					n.entriesMaxY[0],
					minX, minY, maxX, maxY);
			...
			for (int i = 1; i < n.entryCount; i++) {
				...
				if ((tempEnlargement < leastEnlargement) ||
						((tempEnlargement == leastEnlargement) &&
								(Rectangle.area(tempMinX, tempMinY, tempMaxX, tempMaxY) < Rectangle.area(
										n.entriesMinX[index], n.entriesMinY[index], n.entriesMaxX[index],
										n.entriesMaxY[index])))) {
					index = i;
		...
		for (int i = 0; i < n.entryCount; i++) {
			...
			if (n.level > 1) { // if not a leaf
				if (!checkConsistency(n.ids[i], n.level - 1,
						new Rectangle(n.entriesMinX[i], n.entriesMinY[i], n.entriesMaxX[i], n.entriesMaxY[i]))) {
					return false;
				}
			}
		}
